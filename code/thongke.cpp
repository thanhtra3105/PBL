...
void thongke::luukho()
{
    ...
    if (xuat.ngay >= nhap.ngay)
        luu.ngay = xuat.ngay - nhap.ngay;
    else
        luu.ngay = nhap.ngay - xuat.ngay;

    if (xuat.thang >= nhap.thang)
        luu.thang = xuat.thang - nhap.thang;
    else
        luu.thang = nhap.thang - xuat.thang;

    luu.nam = nhap.nam - xuat.nam;
    tg = luu.ngay + luu.thang * 30 + luu.nam * 365;
}
...
void thongke::ht_date_n()
{
    cout << nhap.ngay << "/" << nhap.thang << "/" << nhap.nam;
}
void thongke::ht_date_x()
{
    cout << xuat.ngay << "/" << xuat.thang << "/" << xuat.nam;
}
...
void thongke::ghi(fstream &g)
{
    ...
    g.write(reinterpret_cast<char *>(&luu), sizeof(thoigian));
    g.write(reinterpret_cast<char *>(&tg), sizeof(long long));
}
...
void thongke::doc(fstream &d)
{
    ...
    d.read(reinterpret_cast<char *>(&luu), sizeof(thoigian));
    d.read(reinterpret_cast<char *>(&tg), sizeof(long long));
}
